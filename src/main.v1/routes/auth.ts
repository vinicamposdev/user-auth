...
import {
  forgotPassword, login, refreshToken, register, resetPassword
...
export default (app: any) => {
  ...
  router.post('/refresh-token', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { authorization } = req.headers; // TODO: verify from authentication header
      if (!authorization) return res.status(403).json({ message: 'Need a token' });
      // call refresh token controller
      const response: any = await refreshToken(authorization);
